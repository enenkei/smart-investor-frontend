...
interface ETFDiscoveryTabsProps {
    onSelectSymbol: (symbol: string) => void;
    selectedSymbol: string | null;
}

export function ETFDiscoveryTabs({ onSelectSymbol, selectedSymbol }: ETFDiscoveryTabsProps) {
    ...
    React.useEffect(() => {
        const fetchDiscovery = async () => {
            try {
                const res = await fetch("/api/portfolio/etf-screener");
                if (res.ok) {
                    const data = await res.json();
                    setDiscoveryData(data);
                }
            } catch (err) {
                console.error("Failed to fetch ETF Screener discovery data:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <Card className="rounded-none border-border/50 bg-card/25 backdrop-blur-md p-8 flex flex-col items-center justify-center min-h-[250px]">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-muted-foreground">Aggregating Global ETF Metrics...</span>
            </Card>
        );
    }
    ...
    const tabsList = [
        ...
        { id: "growth", label: "Growth Compounders", icon: TrendingUp, color: "text-violet-400 border-violet-500/20" },
        { id: "dividends", label: "High Dividends", icon: DollarSign, color: "text-amber-400 border-amber-500/20" },
        { id: "hidden_gems", label: "Boutique Hidden Gems", icon: Gem, color: "text-cyan-400 border-cyan-500/20" },
    ] as const;

    const currentList = discoveryData[activeTab] || [];

    return (
        <Card className="rounded-none border-border/50 bg-card/20 backdrop-blur-xl shadow-2xl overflow-hidden">
            <CardHeader className="border-b border-border/50 bg-muted/5 p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-primary">Unified ETF Discovery Board</span>
                    <CardTitle className="text-lg font-black uppercase tracking-tight italic text-foreground mt-0.5">
                        Market-Curated Top Picks
                    </CardTitle>
                </div>
            ...
            <CardContent className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {currentList.slice(0, 4).map((etf: any, idx: number) => {
                        const isSelected = selectedSymbol === etf.symbol;
                        return (
                            <motion.div
                                key={etf.symbol}
                            ...
                            >
                                <div className="space-y-1">
                                    ...
                                    <h4 className="font-black text-sm italic text-foreground tracking-tight line-clamp-2 mt-2">{etf.name}</h4>
                                    <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider">
                                        {etf.category || "General ETF"}
                                    </p>
                                </div>
                                ...
                                <div className="mt-4 pt-3 border-t border-border/30 flex items-end justify-between">
                                    ...
                                    <div className="flex flex-col items-end">
                                        <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground">
                                            {activeTab === "best_performers" && "Total Return"}
                                        ...
                                        <span className={`font-mono text-sm font-black ${
                                            ...
                                            {activeTab === "best_performers" && `+${Number(etf.total_return || etf.score).toFixed(1)}%`}
                                            {activeTab === "growth" && `${Number(etf.growth_score || etf.total_return).toFixed(1)}`}
                                            {activeTab === "dividends" && `${Number(etf.dividend_yield).toFixed(2)}%`}
                                            {activeTab === "hidden_gems" && `${Number(etf.quality_score || etf.total_return).toFixed(1)}`}
                                        </span>
                                    </div>
                                </div>
                            </motion.div>
